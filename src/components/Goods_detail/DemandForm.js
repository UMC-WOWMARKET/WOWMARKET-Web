  ...
  return (
    ...
    >
      ...
      <div className="item3">
        ...
        <div className="common-box button" style={{ margin: "auto 0 auto 0" }}>
          {count}
        </div>
        <div
          className="common-box button"
...
const DemandForm = ({ goods_id }) => {
  ...
  //총 수량
  const totalCount = Object.values(itemQuantities).reduce(
  ...
  const handleAddToOrder = (demandItemId, count) => {
    const existingOrderIndex = orderList.findIndex(
      (order) => order.demandItemId === demandItemId
    );
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `https://www.wowmkt.kr/demand_project/${goods_id}/item`
      );
