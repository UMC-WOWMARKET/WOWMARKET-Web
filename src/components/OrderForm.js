...
const SelectItem = ({ id, index, name, price, onChangeQuantity }) => {
  ...
  return (
    ...
    >
      ...
      <div className="item3">
        <div
          className="common-box button"
          onClick={handleDecrease}
          style={{ margin: "auto 0 auto 12px" }}
        >
          -
        </div>
        <div className="quantity-button" style={{ margin: "auto 0 auto 0" }}>
          {quantity}
        </div>
        <div
          className="common-box button"
          onClick={handleIncrease}
          style={{ margin: "auto 27px auto 0" }}
        >
          +
        </div>
      </div>
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `https://www.wowmkt.kr/project/${goods_id}/item`
      );
      ...
      <div className="OrderForm">
        ...
        <div className="deliveryMethod">
          <div className="section1">
            ...
            <p className="redText" style={{ margin: "39px 0 18px 9px" }}>
              판매자가 지정한 수령방법입니다.
            </p>
          </div>
          <div className="section2">
            <div
              className={`common-box ${
                receiveType === "delivery" ? "selected" : ""
              }`}
              style={{ width: "200px", height: "80px" }}
            >
              <div className="text1">택배배송</div>
              {receiveType === "delivery" && (
                <div className="text2">택배비 {deliveryFee}원</div>
              )}
            </div>
            <div
              className={`common-box ${
                receiveType === "pickup" ? "selected" : ""
              }`}
              style={{ width: "200px", height: "80px" }}
            >
              <div className="text1">직접수령</div>
              {receiveType === "pickup" && (
                <div className="text2">장소 {receiveAddress}</div>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="orderInfo">
          ...
          <div>
            ...
            <div>
              ...
              <FinalSum>
                {deliveryFee ? deliveryFee + totalPrice : totalPrice}원
              </FinalSum>
            </div>
          </div>
        </div>
        ...
        <div className="deliveryInfo">
          ...
          <div className="common-box" style={{ width: "420px" }}>
            <form>
              ...
              <label className="label" htmlFor="detailAddress">
                상세주소
              </label>
              <input
                className="big-box"
              ...
              <label className="label" htmlFor="deliveryMessage">
                배송메시지
              </label>
              <select
                className="delivbox"
        ...
        <div className="sellerAccountInfo">
          ...
          <div
            className="common-box"
            style={{ width: "420px", height: "98px", flexDirection: "column" }}
          >
        ...
        <div
          className="redText"
          style={{ margin: "10px 54px 30px 0", fontSize: "12px" }}
        >
          환불이 필요한 경우 입력해주신 계좌로 진행됩니다.
        </div>
      </div>
